...
x = Conv2D(256, (1, 1), data_format='channels_first', activation='relu', padding='same')(input_txt)
...
print(np.shape(x))#(?, 16, 14, 14)
input_ins = UpSampling2D((2, 2), data_format='channels_first')(x)
print(np.shape(x))#(?, 16, 28, 28)

decoded = Conv2D(3, (3, 3), data_format='channels_first', activation='sigmoid', padding='same')(input_ins)
...
print(np.shape(x))  # (?, 16, 28, 28)
output_ins = MaxPooling2D((2, 2), data_format='channels_first', padding='same')(x)
print(np.shape(x))  # (?, 16, 14, 14)
x = Conv2D(16, (3, 3), data_format='channels_first', activation='relu', padding='same')(output_ins)
print(np.shape(x))#(?, 16, 28, 28)
...
x = Conv2D(256, (1, 1), data_format='channels_first', activation='relu', padding='same')(x)
print(np.shape(x))
...
autoencoder_mid = Model(input_txt, decoded)
#autoencoder_txt_input = Model(input_txt, input_txt)
#autoencoder_ins = Model(input_ins, output_ins)
autoencoder_txt.compile(optimizer='adadelta', loss='binary_crossentropy')
#autoencoder_ins.compile(optimizer='adadelta', loss='binary_crossentropy')

with io.open('C:/MLProjects/venv7/000111.txt', 'r') as f:
    text = f.read().split('\n')
...
decoded_imgs = autoencoder_mid.predict(x_train)
